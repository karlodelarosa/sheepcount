...
import { Home, MapPin, Users, Plus, Calendar, ArrowLeft, BookOpen, UserPlus, X, Check, Edit2 } from "lucide-react";
import { mockHouseholds, mockPeople, mockBibleStudyMembers, mockBibleStudyGroups } from "@/components/mock-data";
...
function SetRoleDialog({ open, onOpenChange, person, onRoleSet }: SetRoleDialogProps) {
...
export function HouseholdDetails({ householdId, onBack }: HouseholdDetailsProps) {
  ...
  const otherResidents = mockNonMembers.filter(nm => nm.householdId === householdId);
  ...
  if (!household) {
    return (
        <div className="space-y-6">
            ...
            <Card><CardContent className="p-8 text-center text-slate-500">The requested household could not be found.</CardContent></Card>
        </div>
    );
  }
  ...
  const householdGroups = mockBibleStudyMembers
    ...
    .reduce((acc, bsm) => {
        ...
        if (acc.has(bsm.bibleStudyId)) {
            ...
            const personName = mockPeople.find(p => p.id === bsm.personId)?.name || 'Unknown';
  ...
  return (
            ...
            <Card className="border-slate-200/60 bg-white md:col-span-2">
                <CardHeader>
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-lg">Bible Study Group Status</CardTitle>
                    </div>
                </CardHeader>
                <CardContent>
                    {bibleStudyGroups.length > 0 ? (
                        <div className="space-y-3">
                            {bibleStudyGroups.map((group, index) => (
                                <div key={index} className="p-4 bg-green-50 rounded-xl border border-green-200/60">
                                    <div className="flex items-center justify-between">
                                        <p className="text-green-800 font-medium">Group: <span className="text-green-600">{group.name}</span></p>
                                        <Badge className="bg-green-100 text-green-700 rounded-lg"><Check className="w-4 h-4 mr-1" /> Active</Badge>
                                    </div>
                                    <p className="text-green-700 text-sm mt-1">
                                        {group.membersCount} household member(s) belong to this group: {group.memberNames.join(', ')}
                                    </p>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="p-4 bg-yellow-50 rounded-xl border border-yellow-200/60 flex items-center justify-between">
                            <p className="text-slate-900 font-medium">Not yet assigned to a Bible Study group.</p>
                            <Button variant="outline" size="sm" className="rounded-lg border-yellow-500 text-yellow-700 hover:bg-yellow-100">Assign Group</Button>
                        </div>
                    )}
                </CardContent>
            </Card>
                            ...
                            <TableBody>
                                {members.map((member) => (
                                    <TableRow key={member.id}>
                                        ...
                                        <TableCell>
                                            <Button variant="ghost" size="sm" className="rounded-lg" onClick={() => handleOpenRoleDialog(member)}>
        ...
        <Card className="border-slate-200/60 bg-white">
            <CardHeader>
                <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-lg">Other Residents ({otherResidents.length})</CardTitle>
                </div>
                <CardDescription>People living here who are not current church members.</CardDescription>
            </CardHeader>
            <CardContent>
    ...
    <Button 
        ...
        onClick={() => router.push(`/evangelism/prospects/${resident.id}`)}
    >
        View/Follow Up
    </Button>
</TableCell>
