...
import { X, UserPlus, Search, ArrowLeft, Shield } from "lucide-react"; // Shield for Ministry icon
import { mockMinistryAssignments, mockPeople, mockMinistries } from "@/components/mock-data";
...
export function MinistryDetailPage({ ministryId }: MinistryDetailPageProps) {
  ...
  if (!ministry) {
    return (
      <div className="p-8 text-center">
        <h1 className="text-2xl font-bold text-red-600 dark:text-red-400">Ministry Not Found</h1>
        <p className="text-slate-600 dark:text-zinc-400">The ministry ID "{ministryId}" does not exist.</p>
  ...
  // Data Logic
  const assignments = mockMinistryAssignments.filter(a => a.ministryId === ministryId);
  ...
  const handleAddMember = () => {
    if (newPersonId && newRole) {
      console.log("Adding member to ministry:", { ministryId, personId: newPersonId, role: newRole });
      setNewPersonId("");
      setNewRole("");
    }
  };
  ...
  const MinistryColorClass = "from-teal-500 to-teal-700 dark:from-sky-600 dark:to-cyan-800";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-slate-200 dark:border-zinc-700">
        <div className="flex items-center gap-4">
          ...
          <div>
                  ...
                  <p className="text-slate-600 dark:text-zinc-400">Manage members and roles for this team</p>
              </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-1 border-slate-200/60 bg-white dark:border-zinc-700/60 dark:bg-zinc-800 h-fit">
          <CardHeader>
            <CardTitle className="text-slate-900 dark:text-white">Add New Member</CardTitle>
            <CardDescription className="text-slate-600 dark:text-zinc-400">Assign a new person and role to this ministry.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label className="text-slate-700 dark:text-zinc-300">Select Person</Label>
        ...
        <Card className="lg:col-span-2 border-slate-200/60 bg-white dark:border-zinc-700/60 dark:bg-zinc-800">
          <CardHeader>
            <div className="flex items-center justify-between">
                <CardTitle className="text-lg text-slate-900 dark:text-white">Current Members ({members.length})</CardTitle>
            ...
            <div className="border border-slate-200/60 rounded-xl overflow-hidden dark:border-zinc-700/60">
              {filteredMembers.length === 0 ? (
                <div className="p-8 text-center text-slate-500 dark:text-zinc-500">
                  {searchTerm ? "No members found matching your search." : "No members have been assigned yet."}
                </div>
              ) : (
                <div className="divide-y divide-slate-200/60 dark:divide-zinc-700/60 max-h-[60vh] overflow-y-auto">
                  {filteredMembers.map((member) => (
                    <div key={member.id} className="p-4 flex items-center justify-between transition-colors hover:bg-slate-50 dark:hover:bg-zinc-700/50">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${DualModeMemberAvatarClass} flex items-center justify-center shadow-sm`}>
                            <span className="text-white">{member.person?.name?.charAt(0)}</span>
                        </div>
                        <div>
                            <p className="text-slate-900 dark:text-white">{member.person?.name}</p>
                            <p className="text-slate-600 dark:text-zinc-400">{member.person?.householdName}</p>
                        </div>
                      </div>
