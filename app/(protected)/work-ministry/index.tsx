...
import { mockMinistries, mockMinistryAssignments, mockPeople } from "@/components/mock-data";
...
export function MinistriesView() {
  ...
  const DualModeOutlineBadgeClass = "rounded-lg border-slate-300 text-slate-600 dark:border-zinc-700 dark:text-zinc-400";
      ...
      <Card className="border-slate-200/60 bg-white/50 backdrop-blur-sm dark:border-zinc-700/60 dark:bg-zinc-800/50">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-slate-900 dark:text-white">Work Ministry Assignments</CardTitle>
              <CardDescription className="text-slate-600 dark:text-zinc-400">Manage ministry teams and member assignments (people can serve in multiple ministries)</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {mockMinistries.map((ministry) => {
              ...
              const colorClass = colorClasses[ministry.color as keyof typeof colorClasses] || colorClasses.purple;
              ...
              return (
                ...
                >
                  <CardHeader>
                    ...
                    <CardTitle className="text-slate-900 dark:text-white">{ministry.name}</CardTitle>
                    <CardDescription className="text-slate-600 dark:text-zinc-400">{ministry.description}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between pb-2 border-b border-slate-200/60 dark:border-zinc-700/60">
                        <span className="text-slate-600 dark:text-zinc-400">Team Members</span>
                        <Badge variant="secondary" className={DualModeSecondaryBadgeClass}>
                          {members.length}
                        </Badge>
                      </div>
                      ...
                      {members.length > 0 ? (
                        <div className="space-y-2">
                          {members.slice(0, 3).map((member) => (
                            <div key={member.id} className="flex items-center justify-between text-slate-900 dark:text-white">
                              ...
                              <Badge variant="outline" className={DualModeOutlineBadgeClass}>
                                {member.role}
                              </Badge>
                            </div>
                          ))}
                        ...
                        <p className="text-slate-500 text-center py-4 dark:text-zinc-500">No members assigned</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
