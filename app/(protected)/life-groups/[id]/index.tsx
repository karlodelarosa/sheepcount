...
import { X, UserPlus, Search, ArrowLeft, UserCircle, Users } from "lucide-react";
import { mockLifeGroupMembers, mockPeople, mockLifeGroups } from "@/components/mock-data";
...
export function LifeGroupDetails({ groupId, onBack }: LifeGroupDetailsProps) {
  ...
  if (!group) {
    return (
        <div className="space-y-6 p-6">
            ...
            <Card><CardContent className="p-8 text-center text-slate-500">The requested life group could not be found.</CardContent></Card>
        </div>
    );
  }
  ...
  const handleAddMember = () => {
    if (newPersonId) {
      console.log("Adding member to life group:", { groupId, personId: newPersonId });
      setNewPersonId("");
    }
  };
  ...
  const colorClass = group.color as keyof typeof group.color ? "from-purple-500 to-purple-700 dark:from-indigo-500 dark:to-indigo-700" : "from-purple-500 to-purple-700 dark:from-indigo-500 dark:to-indigo-700";
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
              <div>
                  ...
                  <p className="text-slate-600 dark:text-zinc-400">{group.description}</p>
              </div>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-1 border-slate-200/60 bg-white dark:border-zinc-700/60 dark:bg-zinc-800 h-fit">
          <CardHeader>
            ...
            <CardDescription className="text-slate-600 dark:text-zinc-400">Select a person to add to this life group.</CardDescription>
          </CardHeader>
                  ...
                  {filteredMembers.map((member) => (
                    <div key={member.id} className="p-4 flex items-center justify-between transition-colors hover:bg-slate-50 dark:hover:bg-zinc-700/50">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-slate-900 dark:text-white font-medium">{member.person?.name}</p>
                          <p className="text-slate-600 dark:text-zinc-400 text-sm">{member.person?.householdName}</p>
                        </div>
                      </div>
