...
import { mockLifeGroups, mockLifeGroupMembers, mockPeople } from "@/components/mock-data";
...
export function LifeGroupsList() {
  ...
  const DualModePrimaryButtonClass = "rounded-xl text-white shadow-lg \
  ...
  const getGroupMembers = (groupId: string) => {
    ...
    const memberships = mockLifeGroupMembers.filter(m => m.lifeGroupId === groupId);
  ...
  const groupedByCategory = mockLifeGroups.reduce((acc, group) => {
    // ... (logic remains the same)
  ...
  const colorClasses = {
    ...
    green: "from-green-500 to-green-700 dark:from-emerald-500 dark:to-emerald-700",
    purple: "from-purple-500 to-purple-700 dark:from-indigo-500 dark:to-indigo-700",
    pink: "from-pink-500 to-pink-700",
    indigo: "from-indigo-500 to-indigo-700 dark:from-violet-500 dark:to-fuchsia-700",
  };
  ...
  const categoryColors = {
    ...
    Adults: "from-purple-500 to-indigo-600 dark:from-violet-500 dark:to-fuchsia-700",
  };

  const DualModeMemberAvatarClass = "from-slate-900 to-slate-700 dark:from-zinc-700 dark:to-zinc-500";
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-slate-600 dark:text-zinc-400">Fellowship groups and program attendance tracking</CardDescription>
            </div>
          ...
          {Object.entries(groupedByCategory).map(([category, groups]) => (
            <div key={category}>
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${categoryColors[category as keyof typeof categoryColors]} flex items-center justify-center shadow-sm`}>
                  <Users className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h3 className="text-slate-900 dark:text-white">{category}</h3>
                  <p className="text-slate-600 dark:text-zinc-400">{groups.length} {groups.length === 1 ? 'group' : 'groups'}</p>
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <CardHeader>
                        ...
                        <CardTitle className="text-slate-900 dark:text-white">{group.name}</CardTitle>
                        <CardDescription className="text-slate-600 dark:text-zinc-400">{group.description}</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-3">
                          <div className="flex items-center justify-between pb-2 border-b border-slate-200/60 dark:border-zinc-700/60">
                            <span className="text-slate-600 dark:text-zinc-400">Members</span>
                            <Badge 
                              variant="secondary" 
                              className="rounded-lg bg-slate-100 text-slate-700 dark:bg-zinc-700 dark:text-zinc-300"
                            >
                          ...
                          {members.length > 0 ? (
                            <div className="space-y-2">
                              {members.slice(0, 3).map((member) => (
                                <div key={member.id} className="flex items-center gap-2 text-slate-900 dark:text-white">
                              ...
                              {members.length > 3 && (
                                <p className="text-slate-500 dark:text-zinc-500">+{members.length - 3} more</p>
                              )}
                            </div>
                          ) : (
                            <p className="text-slate-500 text-center py-4 dark:text-zinc-500">No members yet</p>
                          )}
                        </div>
                      </CardContent>
                    </Card>
