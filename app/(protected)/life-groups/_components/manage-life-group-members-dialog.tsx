...
interface ManageLifeGroupMembersDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  groupId: string | null;
}

export function ManageLifeGroupMembersDialog({ open, onOpenChange, groupId }: ManageLifeGroupMembersDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[700px] bg-white border-slate-200/60 dark:bg-zinc-800 dark:border-zinc-700/60 dark:text-white">
        <DialogHeader>
          <DialogTitle className="text-slate-900 dark:text-white">Manage Life Group Members</DialogTitle>
          <DialogDescription className="text-slate-600 dark:text-zinc-400">
            {group ? `Managing members for ${group.name}` : "Manage life group members"}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-slate-900 dark:text-white">Current Members ({members.length})</h3>
              ...
              {filteredMembers.length === 0 ? (
                ...
                <div className="divide-y divide-slate-200/60 dark:divide-zinc-700/60">
                  {filteredMembers.map((member) => (
                    <div key={member.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-zinc-800/50">
