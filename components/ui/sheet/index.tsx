...
function SheetContent({
  ...
  // Detect if we are in Storybook
  const isStorybook = typeof window !== "undefined" && window.location.hostname.includes("localhost");
    ...
    <>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          ...
          side === "top" &&
            "inset-x-0 top-0 w-full max-h-[70vh] border-b data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top",
          side === "bottom" &&
            "inset-x-0 bottom-0 w-full max-h-[70vh] border-t data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
          className
        )}
        {...props}
      >
        ...
        <SheetPrimitive.Close className="absolute top-4 right-4 opacity-70 hover:opacity-100 rounded-xs focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-ring">
  ...
  return isStorybook ? Content : <SheetPortal>{Content}</SheetPortal>;
}
